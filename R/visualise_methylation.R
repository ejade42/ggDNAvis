...
#' @param index_annotation_always_first_base `logical`. Whether to force the first base in each line to always be annotated regardless of whether it occurs at the `index_annotation_interval`. Defaults to `FALSE`.
...
visualise_methylation <- function(
    ...
    not_na <- list(modification_locations = modification_locations, modification_probabilities = modification_probabilities, sequence = sequences, background_colour = background_colour, other_bases_colour = other_bases_colour, low_colour = low_colour, high_colour = high_colour, low_clamp = low_clamp, high_clamp = high_clamp, sequence_text_type = sequence_text_type, sequence_text_rounding = sequence_text_rounding, sequence_text_colour = sequence_text_colour, sequence_text_size = sequence_text_size, index_annotation_colour = index_annotation_colour, index_annotation_size = index_annotation_size, index_annotation_interval = index_annotation_interval, index_annotations_above = index_annotations_above, index_annotation_vertical_position = index_annotation_vertical_position, index_annotation_full_line = index_annotation_full_line, index_annotation_always_first_base = index_annotation_always_first_base, margin = margin, return = return, force_raster = force_raster, pixels_per_base = pixels_per_base)
    ...
    bool <- list(return = return, index_annotations_above = index_annotations_above, index_annotation_full_line = index_annotation_full_line, index_annotation_always_first_base = index_annotation_always_first_base, force_raster = force_raster)
    ...
    monitor_time <- monitor(monitor_performance, start_time, monitor_time, "duplicating original vectors")
    ...
    ## Otherwise slow geom_tile
        ...
        if (length(index_annotation_lines) > 0) {
            ...
            index_annotation_data <- rasterise_index_annotations(new_sequences_vector = sequences, original_sequences_vector = sequences_original, index_annotation_lines = index_annotation_lines, index_annotation_interval = index_annotation_interval, index_annotation_full_line = index_annotation_full_line, index_annotations_above = index_annotations_above, index_annotation_vertical_position = index_annotation_vertical_position, index_annotation_always_first_base = index_annotation_always_first_base)
...
#'
#' @param modification_data `dataframe`. A dataframe that must contain columns for methylation locations, methylation probabilities, sequence, and sequence length for each read. The former two should be condensed strings as produced by [vector_to_string()] e.g. `"1,2,3,4"`.\cr\cr See [`example_many_sequences`] for an example of a compatible dataframe.
