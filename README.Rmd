...
output: 
    bookdown::github_document2:
        ...
        toc: true
        toc_depth: 3
bibliography: README_files/2025-07-28 notch2nlc papers.bib
csl: README_files/apa.csl
---

# ggDNAvis IMPORTANT NEED TO REMOVE THESE SECTION NAMES THEY DONT WORK{#intro} 
...
## Introduction to `example_many_sequences` {#example-many-sequences}
...
## Introduction to `string_to_vector()` and `vector_to_string()` {#vector-string-processing}
...
## Loading from FASTQ and metadata file {#loading-from-fastq}

### Standard FASTQ {#standard-fastq}
...
### Modified FASTQ (e.g. methylation) {#modified_fastq}
...
Modification types, locations, and probabilities are all stored as comma-condensed strings produced from vectors via `vector_to_string()`. These can be converted back to vectors via `string_to_vector()` - see \@ref(vector-string-processing).
